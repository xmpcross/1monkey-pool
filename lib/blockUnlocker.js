...
function runInterval(){
    async.waterfall([
        ...
        function(blocks, callback){
            async.filter(blocks, function(block, callback){
                apiInterfaces.rpcDaemon('getblockheaderbyheight', {height: block.height}, function(error, result){
                    if (error){
                        ...
                        return callback('Error with getblockheaderbyheight RPC request for block ' + block.height);
                    }
                    if (!result.block_header){
                        ...
                        block.unlocked = false;
                        return callback('no details returned from getblockheaderbyheight RPC call for block ' + block.height);
                    }
                    var blockHeader = result.block_header;
                    block.orphaned = blockHeader.hash === block.hash ? 0 : 1;
                    block.unlocked = blockHeader.depth >= (config.blockUnlocker.depth - 1);
                    block.reward = blockHeader.reward;
                    callback(null, block.unlocked);
                });
