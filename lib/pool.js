...
var maxExtraNonce = Math.pow(2, 32);
...
var poolAddressPrefix = cnUtil.address_decode(new Buffer(config.poolServer.poolAddress)).toString();
...
if (!varDiffSettings.extraNonceMultiplier) varDiffSettings.extraNonceMultiplier = 1;
if (!varDiffSettings.extraNonceRandRange) varDiffSettings.extraNonceRandRange = 32;
...
setInterval(function(){
    ...
    log('info', logSystem, 'Workers connected: %d', [Object.keys(connectedMiners).length]);
...
function getRandomExtraNonce(extraNonce) {
    var randomExtraNonce = Math.max(Math.round(Math.random() * varDiffSettings.extraNonceRandRange) * varDiffSettings.extraNonceMultiplier, varDiffSettings.extraNonceMultiplier);
    var newExtraNonce = extraNonce + randomExtraNonce;

    return newExtraNonce < maxExtraNonce ? newExtraNonce : newExtraNonce - maxExtraNonce;
}
...
function BlockTemplate(template){
    ...
    this.extraNonce = getRandomExtraNonce(0);
...
BlockTemplate.prototype = {
    nextBlob: function(){
        this.extraNonce = getRandomExtraNonce(this.extraNonce);
...
function jobRefresh(state){
    ...
    case "get_template":
        getBlockTemplate(function(error, result){
            ...
            if (!currentBlockTemplate || currentBlockHeight > currentBlockTemplate.height){
...
function Miner(id, login, workerName, pass, ip, startingDiff, noRetarget, pushMessage, agent){
    this.id = id;
    this.login = login;
    this.pass = pass;
    this.ip = ip;
    ...
    this.difficulty = this.minDiff = parseInt(startingDiff);
    ...
    if (agent && agent.includes('xmr-node-proxy')) {
        ...
        this.difficulty = config.poolServer.nicehashDiff || 400000;
    }
...
Miner.prototype = {
    ...
    getJob: function(forceNew){
        if (this.lastBlockHeight === currentBlockTemplate.height
        ...
        var blob;
        if (!this.proxy) {
            blob = currentBlockTemplate.nextBlob();
        }
        else {
            if (this.pendingDifficulty) {
                this.lastDifficulty = this.difficulty;
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    log('info', logSystem, 'Accepted %s share at difficulty %d/%d - %s%% job %s%% block - from %s@%s id: %d',
        [shareType, shareDiff, job.difficulty, (shareDiff*100/job.difficulty).toFixed(2), (shareDiff*100/blockTemplate.difficulty).toFixed(2), miner.login, miner.ip, miner.id]
    );
...
function getShareBuffer(miner, job, blockTemplate, params) {
    ...
    try {
        ...
        log('error', logSystem, "Can't constructNewBlob with %s nonce from miner %s: %s", [nonce, miner.id, e]);
        return null;
    }
}
...
function processShare(miner, job, blockTemplate, params){
    ...
    log('debug', logSystem, "Processing share: %j", [{job: job, params: params, blockTemplate: {reserveOffset: blockTemplate.reserveOffset, blob: blockTemplate.buffer.toString('hex')}}]);
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                log('info', logSystem,
                    'Block %s found with diff %d at height %d diff %d by miner %s@%s - submit result: %j',
                    [blockFastHash.substr(0, 6), hashDiff, job.height, blockTemplate.difficulty, miner.login, miner.ip, result]
                );
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            miner = new Miner(minerId, login, workerName, params.pass, ip, difficulty, noRetarget, pushMessage, params.agent);
            ...
            log('info', logSystem, 'Miner connected %s@%s worker name: %s diff %d (%s) id: %d', [login, miner.ip, workerName, difficulty, (noRetarget ? 'F' : 'V'), minerId]);
            break;
        case 'getjob':
            if (!miner){
        ...
        case 'submit':
            ...
            if (miner.validShares > 1) {
                log('info', logSystem, 'Miner %d: share took %d secs, valid shares: %d', [miner.id, shareTimeDiff, miner.validShares]);
            }
...
function startPoolServerTcp(callback){
    async.each(config.poolServer.ports, function(portData, cback){
        var handleMessage = function(socket, jsonData, pushMessage){
            ...
            var remote_addr = ip + ':' + socket.remotePort;
            log('debug', logSystem, 'Received %j from %s', [jsonData, remote_addr]);
            ...
            var sendReply = function(error, result){
                ...
                log('debug', logSystem, 'Sending %s to %s', [sendData, remote_addr]);
            };

            handleMinerMethod(jsonData.method, jsonData.params, ip, portData, sendReply, pushMessage);
        };
