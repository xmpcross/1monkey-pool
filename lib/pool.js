...
var poolWalletAddress = rotatingWallet ? null : config.poolServer.poolAddress;
...
function getBlockTemplate(callback){
    apiInterfaces.rpcDaemon('getblocktemplate', {reserve_size: alternateReserveSize ? 17 : 8, wallet_address: getPoolWalletAddress(false)}, callback);
}
...
function processBlockTemplate(template){
    if (!poolWalletAddress || (Object.keys(connectedMiners).length && rotatingWallet)) {
        poolWalletAddress = getPoolWalletAddress(true);
    }

    currentBlockTemplate = new BlockTemplate(template, poolWalletAddress);
...
function getPoolWalletAddress(forceNew) {
    if (poolWalletAddress && !forceNew) {
...
var poolAddressPrefix = cnUtil.address_decode(new Buffer(getPoolWalletAddress(false))).toString();
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                log('block', logSystem,
                    '##### MONKEY FOUND BLOCK ######## Block %s found with diff %d at height %d diff %d by miner %s wallet % ##### MONKEY FOUND BLOCK ########',
                    [blockFastHash.substr(0, 6), hashDiff, job.height, blockTemplate.difficulty, miner.remote.addr, blockTemplate.wallet]
                );
