...
var reserveSize = (forkId % 2 == 0) ? 16 : 17;
var reserveOffsetOverride = (typeof config.poolServer.reserveOffset == 'number') ? config.poolServer.reserveOffset : null;
switch (reserveSize) {
    case 16:
        var instanceIdOffset = 1, extraNonceOffset = 5, poolNonceOffset = 8, workerNonceOffset = 12, extraRandomOffset = 12;
        break;
    case 17:
        var instanceIdOffset = 0, extraNonceOffset = 0, poolNonceOffset = 8, workerNonceOffset = 12, extraRandomOffset = 4;
        break;
    case 8:
        var instanceIdOffset = 4, extraNonceOffset = 0, poolNonceOffset = 0, workerNonceOffset = 0, extraRandomOffset = 0;
        break;
}
...
process.on('message', function(message) {
    switch (message.type) {
        ...
        case 'setMinTemplateRefresh':
            minTemplateRefresh = parseFloat(message.data);
            break;
        case 'setRotateWalletEffort':
            valRotateWalletEffort = parseInt(message.data);
            log('info', logSystem, 'valRotateWalletEffort set to %d%%', [valRotateWalletEffort]);
            break;
...
function BlockTemplate(template){
    ...
    if (reserveSize == 16) {
        this.buffer.writeUInt8(Math.floor(Math.random() * 4), this.reserveOffset + 11);
    }
...
function processShare(miner, job, blockTemplate, params){
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        recordShareData(miner, job, hashDiff.toNumber(), false, null, shareType, blockTemplate);
        if (minTemplateRefresh === 0 || hashDiff.gt(blockTemplate.difficulty * minTemplateRefresh * 0.01)) {
            miner.pushMessage('job', miner.getJob(true));
        }
    }
