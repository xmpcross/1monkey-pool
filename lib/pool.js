...
var commonNonce_17 = global.globalShareBlockTemplate ? default_commonNonce_17 : threadNonce;
var highestShare = 0;
...
function setNonceOffSet(r, customOffset) {
    ...
    switch (r) {
        ...
        case 17:
        default:
            instanceIdOffset = 0, extraNonceOffset = coExtra || 4, poolNonceOffset = 8, workerNonceOffset = 12, extraRandomOffset = 0;
            commonNonce = [[coCommon || 0, Buffer.from(commonNonce_17, 'hex'), 4]], extraNonceMultiplier = (global.globalShareBlockTemplate ? forkId - 1 : null);
...
function newBlockTemplate(error, result){
    ...
    if (!currentBlockTemplate || result.height > currentBlockTemplate.height || (result.height == currentBlockTemplate.height && result.blocktemplate_blob != currentBlockTemplate.blob)) {
        ...
        log('info', logSystem, 'New block to mine at height %d w/ difficulty of %d wallet %s id %s commonNonce %s', [
            result.height, result.difficulty, currentBlockTemplate.wallet.substr(-6), currentBlockTemplate.idHash, commonNonce_17
        ]);
    }
...
Miner.prototype = {
    retargetHashes : function() {
        ...
        if (this.proxy) {
            ...
            newDiff = Math.floor(this.hashes / ((Date.now() - this.connectTime) / 1000 | 0)) * config.poolServer.varDiff.targetTime;
        }
