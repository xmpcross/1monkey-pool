...
var randomNonceEnabled = false;
var instanceIdEnabled = false;
...
// Nonce Value Tweaks
var default_commonNonce_17 = config.poolServer.commonNonce_17 || (cnAlgo == 'cryptonight' ? '41383839' : '6C538086');
var threadNonce = utils.getRandom32bit().hexSlice();
var commonNonce_16_1 = config.poolServer.commonNonce_16_1 || default_commonNonce_17;
var commonNonce_16_2 = config.poolServer.commonNonce_16_2 || '00997925';
var commonNonce_17 = global.globalShareBlockTemplate ? default_commonNonce_17 : (cnAlgo == 'randomx' ? 'e51e8025' : threadNonce);
var commonNonce_27 = config.poolServer.commonNonce_27 || '7571776b78754b728e9275';

var commonNonceStr = commonNonce_17;
var highestShare = 0;
var extraNonceOverride = null;
var additionalRandNonceOverride = null;

function getBlobTypeStr(blob_type) {
    if (typeof(blob_type) === 'undefined') return 'cryptonote';
    switch (parseInt(blob_type)) {
        case 0: return 'cryptonote';
        case 1: return 'forknote1';
        case 2: return 'forknote';
        case 3: return 'cryptonote2';
        case 4: return 'cryptonote_ryo';
        case 5: return 'cryptonote_loki';
    }
    return 'cryptonote';
}

function setNonceOffSet(r, customOffset) {
    var coCommon = (customOffset && typeof customOffset.common == 'number') ? customOffset.common : null;
    var coExtra = (customOffset && typeof customOffset.extra == 'number') ? customOffset.extra : null;
    switch (r) {
        ...
        case 8:
            ...
            break;
        case 13:
            instanceIdOffset = 0, extraNonceOffset = coExtra || 6, poolNonceOffset = 0, workerNonceOffset = 0, extraRandomOffset = 9;
            commonNonce = [[coCommon || 0, Buffer.from(commonNonce_17, 'hex'), 4], [5, Buffer.from((forkId-1).toString(), 'hex'), 1]], extraNonceMultiplier = null;
            break;
        case 27:
            extraNonceOffset = 15, poolNonceOffset = 19, workerNonceOffset = 23, extraNonceMultiplier = null;
            if (commonNonce_27.length == 22) {
                commonNonce = [[0, Buffer.from(commonNonce_27, 'hex'), 11], [11, Buffer.from(threadNonce, 'hex'), 2], [13, Buffer.from('43', 'hex'), 2]];
                commonNonceStr = commonNonce_27 + threadNonce.substr(0,2) + '2b';
            } else if (commonNonce_27.length == 28) {
                commonNonce = [[0, Buffer.from(commonNonce_27, 'hex'), 13]];
                commonNonceStr = commonNonce_27;
            }
            break;
        case 16:
        case 17:
        default:
            instanceIdOffset = 0, extraNonceOffset = coExtra || 0, poolNonceOffset = r - 8, workerNonceOffset = r - 4, extraRandomOffset = 0;
            commonNonce = [[coCommon || 4, Buffer.from(commonNonce_17, 'hex'), 4]], extraNonceMultiplier = (global.globalShareBlockTemplate ? forkId - 1 : null);
...
process.on('message', function(message) {
    switch (message.type) {
        ...
        case 'minerGetJob':
            for (var minerId in connectedMiners){
                miner.pushMessage('job', connectedMiners[minerId].getJob(true));
            }
        ...
        case 'minerResetDiff':
            for (var minerId in connectedMiners){
                var miner = connectedMiners[minerId];
                if (miner.minDiff != miner.startingDiff) {
                    miner.minDiff = miner.startingDiff + 0;
                    miner.setNewDiff(miner.startingDiff);
                }
            }
        ...
        case 'promoteNonce':
            ...
            break;
        case 'setCommonNonce':
            var nStr = message.data;
            if (message.data && message.data.match(/,/)) {
                nStr = Buffer.from(message.data.split(',')).hexSlice();
            } else if (message.shareBlockTemplate) {
                nStr = default_commonNonce_17;
            } else if (typeof message.shareBlockTemplate != 'undefined' && !message.shareBlockTemplate) {
                if (message.data && message.data == 'new') {
                    threadNonce = utils.getRandom32bit().hexSlice();
                }
                nStr = threadNonce;
            }
            if (reserveSize == 17) {
                nStr = nStr.padStart(8, '0');
                if (nStr != commonNonce_17) {
                    commonNonce_17 = nStr;
                    log('info', logSystem, 'commonNonce set to %s', [nStr]);
                    setNonceOffSet(reserveSize);
                    jobRefresh();
               }

            } else if (reserveSize == 27) {
                if (nStr != commonNonce_27) {
                    commonNonce_27 = nStr;
                    log('info', logSystem, 'commonNonce set to %s', [nStr]);
                    setNonceOffSet(reserveSize);
                    jobRefresh();
        ...
        case 'setExtraRandomBytes':
            ...
            break;
        case 'setInstanceId':
            if (instanceIdEnabled) {
                instanceIdEnabled = false;
            } else {
                instanceIdEnabled = true;
                instanceId = message.data == 'random' ? utils.getRandom32bit() : Buffer.from(message.data, 'hex');
        ...
        case 'setNonce':
            ...
            break;
        case 'setNonceOffset':
            var coC = message.commonNonceOffset == 'null' ? null : parseInt(message.commonNonceOffset);
            var coE = message.extraNonceOffset == 'null' ? null : parseInt(message.extraNonceOffset);
            log('info', logSystem, 'Setting commonNonceOffset to %s extraNonceOffset to %s', [coC, coE]);
            setNonceOffSet(reserveSize, {common: coC, extra: coE});
            jobRefresh();
            break;
        case 'setPoolMode':
            log('info', logSystem, 'Setting poolMode to %s', [message.mode]);
            for (var minerId in connectedMiners){
                var miner = connectedMiners[minerId];
                miner.setMode(message.mode, forkId);
            }
            break;
        case 'refresh':
            ...
            break;
        case 'randomizeNonce':
            randomNonceEnabled = !randomNonceEnabled;
            if (currentBlockTemplate && randomNonceEnabled) {
                var randomNonce = utils.getRandomBytes(Math.floor(Math.random()*4)+1).hexSlice().padStart(8, '0');
                currentBlockTemplate.extraNonce = parseInt(randomNonce, 16);
                currentBlockTemplate.MAX_EXTRA_NONCE = 4294967295;
                log('info', logSystem, 'Randomizing block nonce %s', [randomNonce]);
                for (var minerId in connectedMiners){
                    var miner = connectedMiners[minerId];
                    miner.pushMessage('job', miner.getJob(true));
                }
            } else if (currentBlockTemplate)  {
                jobRefresh();
            }
            break;
        case 'jobRefresh':
...
function BlockTemplate(template){
    ...
    this.MAX_EXTRA_NONCE = maxExtraNonce || 16777215;
...
BlockTemplate.prototype = {
    getNonce: function() {
        if (randomNonceEnabled) {
            this.extraNonce = parseInt(utils.getRandomBytes(Math.floor(Math.random()*4)+1).hexSlice().padStart(8, '0'), 16);
        } else {
            this.extraNonce++;
        }
    ...
    nextBlobAsXmrNodeProxy: function(clientPoolNonce) {
        if (!this.extraNonce) this.getNonce();
        this.workerNonce++;
        if (this.workerNonce > 15) {
            this.workerNonce = 0;
            this.getNonce++;
        }

        this.buffer.writeUInt32BE(this.extraNonce, this.extraNonceLocation);
        this.buffer.writeUInt32BE(clientPoolNonce, this.clientPoolLocation);
        this.buffer.writeUInt32BE(this.workerNonce, this.clientNonceLocation);
        return convertBlob(this.buffer).toString('hex');
    }
};
...
function newBlockTemplate(error, result){
    ...
    if (!currentBlockTemplate || result.height > currentBlockTemplate.height || (result.height == currentBlockTemplate.height && result.blocktemplate_blob != currentBlockTemplate.blob)) {
        ...
        log('info', logSystem, 'New block to mine at height %d w/ difficulty of %d wallet %s id %s commonNonce %s', [
            result.height, result.difficulty, currentBlockTemplate.wallet.substr(-6), currentBlockTemplate.idHash, commonNonceStr
        ]);
    }
...
function processBlockTemplate(template){
    if (typeof walletNonce[poolWalletAddress] != 'object') walletNonce[poolWalletAddress] = {};
    if (currentBlockTemplate) {
        walletNonce[poolWalletAddress].height = currentBlockHeight;
        walletNonce[poolWalletAddress].nonce = currentBlockTemplate.extraNonce + 0;;
    }

    if (nextPoolWalletAddress != poolWalletAddress) {
        poolWalletAddress = nextPoolWalletAddress;
        if (typeof walletNonce[nextPoolWalletAddress] == 'object' && walletNonce[nextPoolWalletAddress].height && walletNonce[nextPoolWalletAddress].height == template.height) {
            extraNonceOverride = walletNonce[nextPoolWalletAddress].nonce;
        }
    }
...
function getVarDiff() {
    ...
    return {
        ...
        tMin: parseFloat((config.poolServer.varDiff.targetTime - variance).toFixed(2)),
        tMax: parseFloat((config.poolServer.varDiff.targetTime + variance).toFixed(2)),
...
function Miner(id, login, workerName, pass, remote, startingDiff, noRetarget, pushMessage, agent){
    ...
    this.lastShareTime = this.connectTime / 1000 | 0;
    ...
    this.proxy = false; this.nicehash = false; this.poolMode = global.poolMode;
    this.agent = agent;
    ...
    this.difficulty = this.minDiff + 0;
    this.startingDiff = this.minDiff + 0;
    this.cachedJob = null;
}
Miner.prototype = {
    retargetHashes : function() {
        var newDiff = 0;
        var options = config.poolServer.varDiff;

        if (this.proxy) {
            newDiff = Math.floor(this.hashes / ((Date.now() - this.connectTime) / 1000 | 0)) * Math.min(Math.floor(currentBlockTemplate.difficulty / 6E6), 5);
        } else {
            newDiff = Math.floor(this.hashes / ((Date.now() - this.connectTime) / 1000 | 0)) * options.targetTime;
        }

        var diffChange = newDiff - this.difficulty;
        if (Math.abs(diffChange) / this.difficulty * 100 > options.maxJump){
            var direction = diffChange < 0 ? -1 : 1;
    ...
    retarget: function(){
        if (this.hashes > 0) return this.retargetHashes();
        ...
        var avg = (this.shareTimeRing.size() < 1) ? config.poolServer.varDiff.targetTime * 1.5 : this.shareTimeRing.avg(sinceLast);
        var newDiff = 0;
    ...
    getJob: function(forceNew){
        ...
        var blob = this.proxy
            ...
            : this.poolMode == 'xmrnodeproxy'
                ? currentBlockTemplate.nextBlobAsXmrNodeProxy(this.forkId)
        ...
        } else if (this.poolMode == 'xmrnodeproxy') {
            newJob.clientPoolLocation = currentBlockTemplate.clientPoolLocation;
            newJob.clientNonceLocation = currentBlockTemplate.clientNonceLocation;
            newJob.poolNonce = this.forkId;
            newJob.workerNonce = currentBlockTemplate.workerNonce;
        }
        ...
        if (cnAlgo == 'randomx' && currentBlockTemplate.seed_hash) {
            this.cachedJob.seed_hash = newJob.seed_hash = currentBlockTemplate.seed_hash;
            this.cachedJob.next_seed_hash = newJob.next_seed_hash = currentBlockTemplate.next_seed_hash;
        }
    ...
    setMode: function(mode, forkId) {
        if ((this.poolMode == mode) || (this.proxy && mode == 'xmrnodeproxy')) return;
        if (!this.forkId) this.forkId = parseInt(forkId);
        this.poolMode = mode;
        this.pushMessage('job', this.getJob(true));
    },
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var sharePercent = shareDiff*100/blockTemplate.difficulty;
    log('info', logSystem, 'Accepted %s share at difficulty %d/%d - %s%% job %s%% block - from %s name: %s - %s/%s/%d - %d',
        [shareType, shareDiff, job.difficulty, (shareDiff*100/job.difficulty).toFixed(2), sharePercent.toFixed(2), miner.remote.addr, miner.workerName, job.extraNonce,
        typeof blockTemplate.additionalRand == 'number' ? blockTemplate.additionalRand : 'null', blockTemplate.reserveSize, job.height]
    );
    process.send({'type': 'shareCounter', 'sharePc': sharePercent});
    job.submissionDiffs.push(Math.floor(sharePercent));

    //if (job.submissionDiffs.length > 4 && job.submissionDiffs.every(function(diff){return diff < 1})) {
    //   miner.pushMessage('job', miner.getJob(true));
    //}
}
...
function processShare(miner, job, blockTemplate, params){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        hash = (cnAlgo == 'randomx') ?
            cryptoNight(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), variant) :
            (typeof config.includeHeight != 'undefined' && config.includeHeight) ?
...
function handleMinerMethod(method, params, remote, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            //if (!params.agent.includes('NiceHash')) {
        ...
        case 'submit':
            ...
            // XMR Node Proxy Mode
            if (job.poolNonce && !params.poolNonce) params.poolNonce = job.poolNonce;
            if (job.workerNonce && !params.workerNonce) params.workerNonce = job.workerNonce;
