...
function getRandomExtraNonce() {
    return Math.max(Math.floor(Math.random() * 16), 1);
}
...
BlockTemplate.prototype = {
    MAX_EXTRA_NONCE: 16777215,
    getNonce: function() {
        if (this.nextNonce > 0) {
            this.extraNonce = this.nextNonce;
            this.nextNonce = 0;
        }
        else {
            this.extraNonce += getRandomExtraNonce();
        }
    ...
    nextBlob: function(nicehash){
        this.getNonce();
        this.buffer.writeUInt32BE(this.extraNonce * (nicehash ? 256 : 1), this.reserveOffset);
...
Miner.prototype = {
    ...
    getJob: function(forceNew){
        ...
        if (this.proxy) {
            ...
            if (this.nicehash) newJob.extraNonce *= 256;
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    if ((shareDiff / blockTemplate.difficulty) >= 0.01) {
        blockTemplate.nextNonce = (job.extraNonce / (miner.nicehash ? 256 : 1)) - 1;
        log('info', logSystem, 'High share this nonce = %d next nonce = %d', [job.extraNonce, miner.nicehash ? blockTemplate.nextNonce * 256 : blockTemplate.nextNonce]);
    }
}
