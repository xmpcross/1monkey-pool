...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    log('info', logSystem, 'Accepted %s share at difficulty %d/%d - %s%% job %s%% block - from %s name: %d',
        [shareType, shareDiff, job.difficulty, (shareDiff*100/job.difficulty).toFixed(2), (shareDiff*100/blockTemplate.difficulty).toFixed(2), miner.ip, miner.workerName]
    );
}
