...
function getBlockTemplate(callback){
    apiInterfaces.rpcDaemon('getblocktemplate', {reserve_size: alternateReserveSize ? 17 : 8, wallet_address: getPoolWalletAddress()}, callback);
}
    ...
    case "get_template":
        getBlockTemplate(function(error, result){
            ...
            if (!currentBlockTemplate || currentBlockHeight > currentBlockTemplate.height){
                ...
                processBlockTemplate(result, getPoolWalletAddress());
            }
...
function setWorkerCount() {
	redisClient.hset(config.coin + ':workers', forkId, Object.keys(connectedMiners).length);
}

function getPoolWalletAddress() {
    if (poolWalletAddress) {
        return poolWalletAddress;
    }

    if (typeof config.poolServer.poolAddress == 'string') {
        poolWalletAddress = config.poolServer.poolAddress;
    }
    else if (typeof config.poolServer.poolAddress == 'object' && config.poolServer.poolAddress.constructor.name == 'Array' && config.poolServer.poolAddress.length) {
        var i = (forkId - 1) % config.poolServer.poolAddress.length;
        poolWalletAddress = config.poolServer.poolAddress[i];
    }

    log('info', logSystem, 'Pool wallet set to %s', [poolWalletAddress]);
    return poolWalletAddress;
}
...
var poolAddressPrefix = cnUtil.address_decode(new Buffer(getPoolWalletAddress())).toString();
