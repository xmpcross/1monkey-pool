...
function BlockTemplate(template){
    ...
    this.extraNonce = Math.floor(Math.random() * this.MAX_EXTRA_NONCE);
...
function Miner(id, login, workerName, pass, ip, startingDiff, noRetarget, pushMessage, agent){
    ...
    if (agent && agent.includes('xmr-node-proxy')) {
        ...
        this.nicehash = true;
        this.difficulty = Math.max(this.minDiff, config.poolServer.nicehashDiff || 400000);
    }
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var blockNonce = job.extraNonce / (miner.nicehash ? 256 : 1);
    log('info', logSystem, 'Accepted %s share at difficulty %d/%d - %s%% job %s%% block - nonce %d (%d) - from %s id: %d',
        [shareType, shareDiff, job.difficulty, (shareDiff*100/job.difficulty).toFixed(2), (shareDiff*100/blockTemplate.difficulty).toFixed(2), blockNonce, job.extraNonce, miner.ip, miner.id]
    );
}
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                log('info', logSystem,
                    '##### MONKEY FOUND BLOCK ######## Block %s found with diff %d at height %d diff %d by miner %s id: %s - submit result: %j ##### MONKEY FOUND BLOCK ########',
                    [blockFastHash.substr(0, 6), hashDiff, job.height, blockTemplate.difficulty, miner.ip, miner.id, result]
                );
