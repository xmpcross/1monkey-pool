...
var nextPoolWalletAddress = poolWalletAddress = rotatingWallet ? getRotatingPoolWalletAddress() : config.poolServer.poolAddress;
...
process.on('message', function(message) {
    switch (message.type) {
        ...
        case 'forceRetarget':
            ...
            break;
        case 'setWallet':
            log('info', logSystem, 'Pool wallet changed to %s', [message.wallet]);
            nextPoolWalletAddress = message.wallet;
...
function jobRefresh(state){
    ...
    case "get_template":
        getBlockTemplate(function(error, result){
            ...
            if (!currentBlockTemplate || currentBlockHeight > currentBlockTemplate.height || nextPoolWalletAddress != currentBlockTemplate.wallet){
...
function handleBlockFound() {
    if (hasWalletPool) {
        ...
        process.send({type: 'setWallet', wallet: config.poolServer.wallets[i]});
    }
    else {
        jobRefresh("get_template");
    }
