...
var cryptoNight = multiHashing[config.cnAlgo || 'cryptonight'];
...
var blankExtraRandom = new Buffer(4);
...
var defaultReserveSize = forkId % 2 == 0 ? 17 : 17;
...
var commonNonce_17 = config.poolServer.commonNonce_17 || '004397AF';
var highestShare = 0;
...
function setNonceOffSet(r) {
    switch (r) {
        ...
        case 8:
            ...
            break;
        case 16:
            instanceIdOffset = 0, extraNonceOffset = 9, poolNonceOffset = 8, workerNonceOffset = 12, extraRandomOffset = 0;
            commonNonce = [[1,  new Buffer(commonNonce_16_1, 'hex'), 4], [5,  new Buffer(forkId.toString(16).padStart(8, '0'), 'hex'), 4], [12, new Buffer(commonNonce_16_2, 'hex'), 4]],
            extraNonceMultiplier = (forkId - 1) % 4, maxExtraNonce = 1023;
            //instanceIdOffset = 1, extraNonceOffset = 5, poolNonceOffset = 8, workerNonceOffset = 12, extraRandomOffset = 12;
            break;
        case 17:
        default:
            instanceIdOffset = 0, extraNonceOffset = 1, poolNonceOffset = 8, workerNonceOffset = 12, extraRandomOffset = 0;
            commonNonce = [[4, new Buffer(commonNonce_17, 'hex'), 4]], extraNonceMultiplier = (forkId - 1) * (2 / numForks), maxExtraNonce = 4095;
...
process.on('message', function(message) {
    switch (message.type) {
        ...
        case 'setReserveSize':
            ...
            reserveSize = message.data == 'null' ? defaultReserveSize : parseInt(message.data);
        ...
        case 'setExtraRandomBytes':
            ...
            break;
        case 'setInstanceId':
            instanceId = new Buffer(message.data, 'hex');
...
PoolMiner.prototype = {
    ...
    buildBlockTemplateBuffer: function() {
        this.blockTemplateBuffer = new Buffer(this.blockTemplate.buffer.length);
    ...
    calculateDiff: function(shareBuffer) {
        var convertedBlob = convertBlob(shareBuffer);
        var cnVariant = (config.cnVariantBlock && config.cnVariantBlock < this.blockTemplate.height) ? 1 : 0;
        var hash = cryptoNight(convertedBlob, cnVariant);
        var hashArray = hash.toByteArray().reverse();
        var hashNum = bignum.fromBuffer(new Buffer(hashArray));
    ...
    runMining: function() {
        ...
        var shareBuffer = constructNewBlob(this.blockTemplateBuffer, new Buffer(nonce, 'hex'));
