...
function BlockTemplate(template, wallet){
    /*
    ...
    case "get_template":
        getBlockTemplate(function(error, result){
            ...
            if (!currentBlockTemplate || currentBlockHeight > currentBlockTemplate.height){
                ...
                processBlockTemplate(result, config.poolServer.poolAddress);
            }
...
function processBlockTemplate(template, wallet){
    ...
    currentBlockTemplate = new BlockTemplate(template, wallet);
...
function setWorkerCount() {
	redisClient.hset(config.coin + ':workers', process.env.forkId, Object.keys(connectedMiners).length);
}
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var redisCommands = [
		...
		['zadd', config.coin + ':sharelog', dateNowSeconds, [dateNow, shareDiff, job.difficulty, blockTemplate.difficulty, miner.remote.addr, blockCandidate ? blockTemplate.wallet : ''].join(':')]
    ];
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                log('block', logSystem,
                    '##### MONKEY FOUND BLOCK ######## Block %s found with diff %d at height %d diff %d by miner %s ##### MONKEY FOUND BLOCK ########',
                    [blockFastHash.substr(0, 6), hashDiff, job.height, blockTemplate.difficulty, miner.remote.addr]
                );
...
function handleMinerMethod(method, params, remote, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            log('warn', logSystem, 'Miner connected from %s worker name: %s diff %d (%s) addr: %s', [miner.remote.addr, workerName, difficulty, (noRetarget ? 'F' : 'V'), login]);
            break;
        case 'getjob':
            if (!miner){
        ...
        case 'submit':
            ...
            if (miner.validShares > 1) {
                log('info', logSystem, 'Miner %s: share took %d secs, valid shares: %d', [miner.remote.addr, shareTimeDiff, miner.validShares]);
            }
