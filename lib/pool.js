...
var instanceIdOffset, extraNonceOffset, poolNonceOffset, workerNonceOffset, extraRandomOffset, extraNonceMultiplier, maxExtraNonce;
...
function setNonceOffSet(r) {
    switch (r) {
        case 16:
            ...
            break;
        case 17:
            instanceIdOffset = 4, extraNonceOffset = 1, poolNonceOffset = 8, workerNonceOffset = 12, extraRandomOffset = 0;
            instanceId = new Buffer('0037A582', 'hex'), extraNonceMultiplier = (forkId - 1) * (16 / numForks), maxExtraNonce = 4095;
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    log('info', logSystem, 'Accepted %s share at difficulty %d/%d - %s%% job %s%% block - from %s name: %s - %s/%s',
        ...
        typeof blockTemplate.additionalRand == 'number' ? blockTemplate.additionalRand : 'null']
    );
}
