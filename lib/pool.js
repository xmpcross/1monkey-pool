...
var alternateReserveSize = false;
...
process.on('message', function(message) {
    switch (message.type) {
        ...
        case 'setWallet':
            ...
            break;
        case 'refresh':
            if (message.data.match(/size/i)) {
                alternateReserveSize = !alternateReserveSize;
                log('info', logSystem, 'alternateReserveSize = %s', [alternateReserveSize]);
            }
...
function BlockTemplate(template){
    ...
    this.alternative_reserve_size = alternateReserveSize;

    if (this.alternative_reserve_size) {
...
function processBlockTemplate(template){
    ...
    var previousHeight = currentBlockHeight;
    ...
    for (var minerId in connectedMiners){
        var miner = connectedMiners[minerId];
        if (!miner.validJobs.length || miner.validJobs.slice(-1)[0].height != currentBlockHeight || now - miner.lastShareTime >= 60) {
    ...
    if (forkId == 1 && currentBlockHeight > previousHeight) {
...
Miner.prototype = {
    ...
    getJob: function(forceNew){
        ...
        else if (this.nicehash && alternateReserveSize) {
            newJob.extraNonce *= 256;
        }
...
function getShareBuffer(miner, job, blockTemplate, params) {
    ...
    if (miner.proxy && blockTemplate.alternative_reserve_size) {
...
function processShare(miner, job, blockTemplate, params){
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        recordShareData(miner, job, hashDiff.toNumber(), false, null, shareType, blockTemplate);
        if (hashDiff.gt(blockTemplate.difficulty * 0.01)) {
            jobRefresh('get_template');
        }
    }
