...
var blankExtraRandom = new Buffer(5);
...
var minTemplateRefresh = (typeof config.poolServer.blockTemplateRefresh == 'number') ? config.poolServer.blockTemplateRefresh : 3;
var valRotateWalletEffort = (typeof config.poolServer.rotateWalletEffort == 'number') ? config.poolServer.rotateWalletEffort : 100;

// Block Reserve Size Tweaks
var reserveSize = (typeof config.poolServer.reserveSize == 'number') ? config.poolServer.reserveSize : 17;
var reserveOffsetOverride = (typeof config.poolServer.reserveOffset == 'number') ? config.poolServer.reserveOffset : null;
switch (reserveSize) {
    case 16:
    case 17:
        var poolNonceOffset = 8, workerNonceOffset = 12, extraRandomOffset = reserveSize - 5;
        break;
    case 8:
        var poolNonceOffset = 0, workerNonceOffset = 0, extraRandomOffset = 0;
        break;
}
...
process.on('message', function(message) {
    switch (message.type) {
        ...
        case 'setMinTemplateRefresh':
            minTemplateRefresh = parseInt(message.data);
            break;
        case 'setRotateWalletEffort':
            valRotateWalletEffort = parseInt(message.data);
            log('info', logSystem, 'valRotateWalletEffort set to %d%%', [rotateWalletEffort]);
            break;
        case 'refresh':
            if (message.data.match(/id/i)) {
...
function BlockTemplate(template){
    ...
    this.extraRandomLocation = extraRandomOffset ? this.reserveOffset + extraRandomOffset : 0;
    this.extraRandomBytes = null;
}
...
BlockTemplate.prototype = {
    ...
    writeExtraRandomBytes: function(randomBytes) {
        if (this.extraRandomLocation) {
            if (!randomBytes) {
                randomBytes = this.extraRandomBytes = crypto.randomBytes(5);
            }
            randomBytes.copy(this.buffer, this.extraRandomLocation, 0, 5);
        }
    },
    nextBlob: function(nicehash){
        this.writeExtraRandomBytes(this.extraRandomBytes);
        this.getNonce();
...
function getBlockTemplate(callback){
    apiInterfaces.rpcDaemon('getblocktemplate', {reserve_size: reserveSize, wallet_address: nextPoolWalletAddress}, callback);
}
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    redisClient.multi(redisCommands).exec(function(err, replies){
        ...
        if (blockCandidate){
            ...
            if (hasWalletPool && (totalShares > blockTemplate.difficulty * valRotateWalletEffort * 0.01)) {
...
function getShareBuffer(miner, job, blockTemplate, params) {
    ...
    if (blockTemplate.extraRandomLocation) {
        job.extraRandomBytes.copy(template, blockTemplate.extraRandomLocation, 0, 5);
    }
...
function processShare(miner, job, blockTemplate, params){
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        recordShareData(miner, job, hashDiff.toNumber(), false, null, shareType, blockTemplate);
        if (minTemplateRefresh && hashDiff.gt(blockTemplate.difficulty * minTemplateRefresh * 0.01)) {
            jobRefresh('get_template');
        }
    }
