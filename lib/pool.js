...
process.on('message', function(message) {
    switch (message.type) {
        ...
        case 'refresh':
            ...
            else if (!message.data.match(/template/i)) {
...
function processBlockTemplate(template){
    ...
    for (var minerId in connectedMiners){
        var miner = connectedMiners[minerId];
        if (!miner.validJobs.length || miner.validJobs.slice(-1)[0].height != currentBlockHeight || now - miner.lastShareTime >= 30) {
    ...
    if (forkId == 1 && template.height > currentBlockHeight) {
