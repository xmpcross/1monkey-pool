...
var instanceId = new Buffer('80808080', 'hex');
...
var maxRotateWalletEffort = (typeof config.poolServer.rotateWalletEffort == 'number') ? config.poolServer.rotateWalletEffort : 50;
...
switch (reserveSize) {
    case 16:
    case 17:
        var poolNonceOffset = 8, workerNonceOffset = 12;
        break;
    case 8:
        var poolNonceOffset = 0, workerNonceOffset = 0;
        break;
}
...
process.on('message', function(message) {
    switch (message.type) {
        ...
        case 'setRotateWalletEffort':
            rotateWalletEffort = parseInt(message.data);
            log('info', logSystem, 'maxRotateWalletEffort set to %d%%', [rotateWalletEffort]);
            break;
        case 'refresh':
...
function processBlockTemplate(template){
    ...
    for (var minerId in connectedMiners){
        var miner = connectedMiners[minerId];
        if (!miner.validJobs.length || miner.validJobs.slice(-1)[0].height != currentBlockHeight || now - miner.lastShareTime >= config.poolServer.varDiff.retargetTime) {
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    redisClient.multi(redisCommands).exec(function(err, replies){
        ...
        if (blockCandidate){
            ...
            if (hasWalletPool && (totalShares < blockTemplate.difficulty * maxRotateWalletEffort * 0.01)) {
