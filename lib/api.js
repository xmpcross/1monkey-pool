    ...
    async.parallel({
        pool: function(callback){
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                var data = {
                    stats: replies[2],
                    lastBlockFound: replies[2] ? replies[2].lastBlockFound : null,
                ...
                if (replies[6]) {
                    data.miners = Object.values(replies[6]).reduce(function (total, c) {
                           return total + parseInt(c);
                    }, 0);
                }

                if (replies[11]) {
                    data.currentBlock = replies[11];
                    for (var k in data.currentBlock) {
                        if (!isNaN(data.currentBlock[k])) {
                            data.currentBlock[k] = parseInt(data.currentBlock[k]);
                        }
                    }
                }
...
function handleRpcRequest(rpcPath, rpcData, response) {
    rpcPath = '/' + rpcPath;
    log('debug', logSystem, 'RPC request on path %s with data %s', [rpcPath, rpcData]);
    apiInterfaces.jsonHttpRequest(config.daemon.host, config.daemon.port, rpcData, function(error, reply) {
        response.writeHead(200, {
            'Access-Control-Allow-Origin': '*',
            'Cache-Control': 'no-cache',
            'Content-Type': 'application/json',
...
var server = http.createServer(function(request, response){
    ...
    if (urlParts.pathname) pathParts = urlParts.pathname.split('/').slice(1);

    if (pathParts.length > 1 && pathParts[0] == 'rpc') {
        var rpcData = '';
        if (request.method == 'POST') {
            ...
            request.on('end', function() {
                handleRpcRequest(pathParts[1], rpcData, response);
            });
        } else {
            handleRpcRequest(pathParts[1], rpcData, response);
        }
